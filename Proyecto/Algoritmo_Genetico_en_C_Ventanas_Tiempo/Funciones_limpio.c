...
double evaluar_individuo(int *genotipo, double **distancias, int longitud_genotipo, double ventanas_tiempo[][2]) {
    ...
    for (int i = 0; i < longitud_genotipo; i++) {
        ...
        if (hora_llegada < ventana_inicio) {
            tiempo_acumulado += (ventana_inicio - hora_llegada); // Esperamos hasta el inicio de la ventana
        } else if (ventana_fin < ventana_inicio) { 
            // Caso especial: La ventana entana cruza a medianoche (ej. 22:00 a 02:00) pero llega a las 03:00
            if (hora_llegada > ventana_fin && hora_llegada < ventana_inicio) {
                tiempo_acumulado += (24 - hora_llegada + ventana_inicio); // Esperamos siguiente día
            }
            // Caso en el que llego despues de que se cerro la ventana de tiempo 
        } else if (hora_llegada > ventana_fin) {
            tiempo_acumulado += (24 - hora_llegada + ventana_inicio); // Esperamos al siguiente día
        }
